...
// The messed up part starts at `<div className="max-h-80 overflow-y-auto custom-scrollbar">` and ends at the second `</div>` after it (actually before `</div> </div> </div>` ). Let's replace the whole dropdown container.

content = content.replace(
    /<div className="max-h-80 overflow-y-auto custom-scrollbar">[\s\S]*?<button onClick=\{handleLogout\}/,
